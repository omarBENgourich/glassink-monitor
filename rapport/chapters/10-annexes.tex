...
\section{Liste de contrôle avant démonstration}

\begin{enumerate}
  \item lancer \texttt{make demo} et attendre l'état sain des six services ;
  \item exécuter \texttt{make verify} sans ignorer un test en échec ;
  \item confirmer que le dernier point InfluxDB date de moins d'une minute ;
  \item ouvrir les vues opérateur, maintenance et direction ;
  \item vérifier la présence des paramètres natifs 9450c ;
  \item ouvrir les quatre onglets Node-RED et le journal de rejet ;
  \item exécuter l'alerte de test vers le canal de démonstration ;
  \item garder le simulateur visible pour expliquer un changement d'état ;
  \item ne jamais supprimer les volumes juste avant une présentation.
\end{enumerate}

\chapter{Procès-verbal de recette à compléter}

\begin{table}[H]
\centering
\caption{Informations de la session de recette}
\begin{tabularx}{\textwidth}{p{0.34\textwidth}Y}
\toprule
Information & Valeur \\
\midrule
Site et ligne & \rule{0.72\linewidth}{0.4pt} \\
Modèle et identifiant de l'imprimante & \rule{0.72\linewidth}{0.4pt} \\
Version du projet & \rule{0.72\linewidth}{0.4pt} \\
Date et heure & \rule{0.72\linewidth}{0.4pt} \\
Représentant production & \rule{0.72\linewidth}{0.4pt} \\
Représentant maintenance & \rule{0.72\linewidth}{0.4pt} \\
Représentant informatique industrielle & \rule{0.72\linewidth}{0.4pt} \\
\bottomrule
\end{tabularx}
\end{table}

\begin{longtable}{p{0.08\textwidth}p{0.47\textwidth}p{0.13\textwidth}p{0.18\textwidth}}
\caption{Feuille de résultat}\\
\toprule
Réf. & Contrôle & Résultat & Observation \\
\midrule
\endfirsthead
\toprule
Réf. & Contrôle & Résultat & Observation \\
\midrule
\endhead
R01 & Les valeurs Grafana correspondent à la machine & \(\square\) OK \(\square\) KO & \\
R02 & L'arrêt sans demande ne crée pas de panne & \(\square\) OK \(\square\) KO & \\
R03 & L'arrêt avec demande déclenche l'alerte & \(\square\) OK \(\square\) KO & \\
R04 & Les consommables et leur lot sont corrects & \(\square\) OK \(\square\) KO & \\
R05 & Un défaut est enregistré avec sa durée & \(\square\) OK \(\square\) KO & \\
R06 & Une perte de flux est signalée & \(\square\) OK \(\square\) KO & \\
R07 & Le service reprend après redémarrage & \(\square\) OK \(\square\) KO & \\
R08 & La sauvegarde de test est restaurée & \(\square\) OK \(\square\) KO & \\
\bottomrule
\end{longtable}

\vspace{1cm}
\noindent\textbf{Décision :}
\(\square\) accepté \quad
\(\square\) accepté avec réserves \quad
\(\square\) refusé

\vspace{1.5cm}
\noindent
\begin{tabularx}{\textwidth}{XXX}
Production & Maintenance & Informatique industrielle \\
\vspace{1.5cm} & & \\
